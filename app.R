        ...
        div(
          div(class="panel",
            div(class="panel-head",
              div(class="panel-head-title","TOP 15 — MASTER SCORE"),
              div(class="panel-head-meta","RANKED BY COMPOSITE SIGNAL")),
            div(class="panel-body", DTOutput("top15_table"))
          )
        ),
        div(
          div(class="panel",
...
server <- function(input, output, session) {
  ...
  output$top15_table <- renderDT({
    if (is.null(master_data)) return(datatable(data.frame(Msg="Run pipeline")))
    df <- master_data %>%
