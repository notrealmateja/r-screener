...
run_module4 <- function(fund_data = NULL) {
  ...
  z_norm <- function(x, higher_is_better = TRUE) {
    ...
    if (length(valid) < 3) return(rep(50, length(x)))
    mu  <- mean(valid)
    sig <- sd(valid)
    ...
    score <- (z + 3) / 6 * 100
    round(ifelse(is.na(x) | !is.finite(x), 50, score), 1)
  }
  ...
  df$z_news    <- z_norm(df$news_score,    TRUE)
  ...
  df <- df %>% mutate(
    ...
    top15_eligible = valuation_ok &
                     !is.na(signal_matches) & signal_matches >= 10 &
                     !is.na(confidence_1d)  & confidence_1d >= 55 &
                     !is.na(master_score),
