run_module1 <- function(tickers=NULL, force_refresh=FALSE) {
  message("\n=== MODULE 1 ===\n")
  library(quantmod);library(dplyr);library(readr);library(jsonlite);library(httr);library(TTR)
  FMP_KEY<-"WEe4bM0zNn8UagrZtzyijnANOJa6qrBK";AV_KEY<-"GCL8H0RSOMX48AWU"
  FH_KEY<-"d6sdol1r01qj447b9nngd6sdol1r01qj447b9no0";POLYGON_KEY<-"CZ9uLCagJ4dWJMs1Efz79gA80ThXlQlO"
  TIINGO_KEY<-"8d2c7e68b5d95ecf39897ec76884bf84f65cf15d";TWELVE_KEY<-"46415134d68a44cb9467d9b1a45e3eea"
  if(!dir.exists("data"))dir.create("data",recursive=TRUE)
  if(!dir.exists("app"))dir.create("app",recursive=TRUE)
  s1<-function(x,default=NA){if(is.null(x)||length(x)==0)return(default);r<-x[[1]];if(is.null(r)||length(r)==0)return(default);r}
  if(is.null(tickers))tickers<-unique(c(
  ...
  cache_file<-"data/fundamentals_scored.csv";batch_file<-"data/batch_tracker.rds"
  ...
  if(!force_refresh&&file.exists(cache_file)&&file.exists(batch_file)){
    ...
    if(tracker$last_run==Sys.Date()){message("Cache current.");return(invisible(read_csv(cache_file,show_col_types=FALSE)))}
  }
  cached<-if(file.exists(cache_file))read_csv(cache_file,show_col_types=FALSE) else NULL
  batch_idx<-today_num%%3;batch_size<-ceiling(length(tickers)/3)
  start_i<-batch_idx*batch_size+1;end_i<-min(start_i+batch_size-1,length(tickers))
  today_tickers<-tickers[start_i:end_i]
  message("Batch ",batch_idx+1,"/3: ",length(today_tickers)," stocks")
  get_fred<-function(id)tryCatch({d<-read_csv(paste0("https://fred.stlouisfed.org/graph/fredgraph.csv?id=",id),show_col_types=FALSE);as.numeric(tail(na.omit(d[[2]]),1))},error=function(e)NA)
  fed_rate<-get_fred("FEDFUNDS");vix_val<-get_fred("VIXCLS")
  yield_spread<-tryCatch(get_fred("DGS10")-get_fred("DGS2"),error=function(e)NA)
  regime<-if(!is.na(vix_val)&&vix_val>30)"RISK_OFF" else if(!is.na(yield_spread)&&yield_spread<0)"INVERTED" else if(!is.na(fed_rate)&&fed_rate>5)"HIGH_RATE" else if(!is.na(fed_rate)&&fed_rate<2)"LOW_RATE" else "NEUTRAL"
  message("Regime: ",regime," Fed: ",fed_rate)
  get_yahoo<-function(sym){
    tryCatch({
      env<-new.env();suppressWarnings(getSymbols(sym,src="yahoo",env=env,auto.assign=TRUE,from=Sys.Date()-380))
      sc<-gsub("\\.","",sym);px_obj<-if(exists(sym,envir=env))get(sym,envir=env) else if(exists(sc,envir=env))get(sc,envir=env) else NULL
      if(is.null(px_obj))return(NULL)
      px<-Cl(px_obj);vol<-Vo(px_obj);if(nrow(px)<60)return(NULL)
      price<-as.numeric(last(px));n<-nrow(px)
      hi52<-as.numeric(max(tail(px,252),na.rm=TRUE));lo52<-as.numeric(min(tail(px,252),na.rm=TRUE))
      r1d<-as.numeric((px[n]/px[max(1,n-1)]-1)*100);r5d<-as.numeric((px[n]/px[max(1,n-5)]-1)*100)
      r1m<-as.numeric((px[n]/px[max(1,n-21)]-1)*100);r3m<-as.numeric((px[n]/px[max(1,n-63)]-1)*100)
      r6m<-as.numeric((px[n]/px[max(1,n-126)]-1)*100);r1y<-as.numeric((px[n]/px[1]-1)*100)
      rsi14<-as.numeric(last(RSI(px,n=14)))
      sma20<-as.numeric(last(SMA(px,n=20)));sma50<-as.numeric(last(SMA(px,n=50)));sma200<-as.numeric(last(SMA(px,n=200)))
      ...
      list(price=round(price,2),high_52w=round(hi52,2),low_52w=round(lo52,2),
           ret_1d=round(r1d,3),ret_5d=round(r5d,3),ret_1m=round(r1m,3),
           ret_3m=round(r3m,3),ret_6m=round(r6m,3),ret_1y=round(r1y,3),
           rsi14=round(rsi14,2),sma20=round(sma20,2),sma50=round(sma50,2),sma200=round(sma200,2),
           macd=round(macd_r,4),macd_signal=round(macd_s,4),bb_pct=round(bb_pct,3),
           vol_20d=round(sd(tail(dr,20),na.rm=TRUE)*sqrt(252)*100,2),
           vol_60d=round(sd(tail(dr,60),na.rm=TRUE)*sqrt(252)*100,2),
           vol_252d=round(sd(dr,na.rm=TRUE)*sqrt(252)*100,2),
           avg_vol_20d=round(mean(as.numeric(tail(vol,20)),na.rm=TRUE),0),
           trend_strength=round(mean(tail(dr,20)>0,na.rm=TRUE)*100,1),
           range_pct=if(!is.na(hi52)&&hi52!=lo52)round((price-lo52)/(hi52-lo52)*100,1) else 50,
           above_ma20=!is.na(sma20)&&price>sma20,above_ma50=!is.na(sma50)&&price>sma50,
  ...
  get_fmp<-function(sym){
    ...
    list(pe_ratio=sn(rat,"peRatioTTM"),pb_ratio=sn(rat,"priceToBookRatioTTM"),
         ...
         interest_coverage=sn(rat,"interestCoverageTTM"),revenue_growth=sn(grw,"revenueGrowth"),
         earnings_growth=sn(grw,"netIncomeGrowth"),fcf_growth=sn(grw,"freeCashFlowGrowth"),
         eps_growth=sn(grw,"epsgrowth"),market_cap=sn(pro,"mktCap"),
         sector=sc(pro,"sector"),industry=sc(pro,"industry"),company=sc(pro,"companyName"),
         dcf_value=sn(dcf,"dcf"))
  }
  get_av<-function(sym)tryCatch({
    ...
    q<-d$quarterlyEarnings;if(is.null(q)||nrow(q)==0)return(list(eps=NA,eps_est=NA,surprise_pct=NA,beat_count=NA))
    ea<-suppressWarnings(as.numeric(q$reportedEPS[1]));ee<-suppressWarnings(as.numeric(q$estimatedEPS[1]))
  ...
  get_finnhub<-function(sym)tryCatch({
    ...
    tot<-sb+b+h+s+ss
    as_s<-if(tot>0)round(((sb*2+b*1+h*0+s*(-1)+ss*(-2))/(tot*2))*50+50,1) else 50
    beta<-tryCatch(as.numeric(met$metric$beta),error=function(e)NA)
    list(analyst_score=as_s,beta=beta,strong_buy=sb,buy=b,hold=h,sell=s,total_analysts=tot,
         short_ratio=tryCatch(as.numeric(met$metric$shortRatio),error=function(e)NA),
         short_pct=tryCatch(as.numeric(met$metric$shortInterestPercentageFreeFloat),error=function(e)NA),
         news_score=NA)
  ...
  get_polygon<-function(sym)tryCatch({
    ...
    Sys.sleep(0.12)
    list(prev_close=if(!is.null(prev$results))prev$results$c[1] else NA,vwap=if(!is.null(prev$results))prev$results$vw[1] else NA,shares_outstanding=NA)
  },error=function(e)list(prev_close=NA,vwap=NA,shares_outstanding=NA))
  get_tiingo<-function(sym)tryCatch({
    ...
    Sys.sleep(0.1);if(is.null(d)||length(d)==0)return(list(adj_close=NA,tiingo_ret1m=NA))
  ...
  get_twelve<-function(sym)tryCatch({
    ...
    Sys.sleep(0.15);list(adx=tryCatch(as.numeric(adx$values$adx[1]),error=function(e)NA))
  },error=function(e)list(adx=NA))
  get_edgar<-function(sym)tryCatch({
    mapping<-fromJSON(content(GET("https://www.sec.gov/files/company_tickers.json"),"text",encoding="UTF-8"))
    ...
    facts<-fromJSON(content(GET(paste0("https://data.sec.gov/api/xbrl/companyfacts/CIK",cik,".json")),"text",encoding="UTF-8"))
    Sys.sleep(0.1)
    ...
    sn_val<-if(!is.null(ni)){an<-ni[ni$form=="10-K",];if(nrow(an)>0)tail(an$val,1) else NA} else NA
    list(sec_revenue=sr,sec_net_income=sn_val)
  ...
  message("Processing ",length(today_tickers)," stocks...")
  today_results<-lapply(seq_along(today_tickers),function(i){
    sym<-today_tickers[i];message("  [",i,"/",length(today_tickers),"] ",sym)
    yah<-yahoo_cache[[sym]]
    if(is.null(yah)||is.na(s1(yah$price))){message("  Skip");return(NULL)}
    fmp<-get_fmp(sym)
    ...
    edgar<-get_edgar(sym)
    dcf_up<-if(!is.na(s1(fmp$dcf_value))&&!is.na(s1(yah$price))&&s1(yah$price)>0)round((s1(fmp$dcf_value)/s1(yah$price)-1)*100,1) else NA
    tryCatch(data.frame(
      ...
      market_cap=s1(fmp$market_cap),shares_outstanding=NA,
      ...
      fed_rate=fed_rate,yield_spread=yield_spread,vix=vix_val,regime=regime,
      data_confidence=50,last_updated=as.character(Sys.Date()),
      stringsAsFactors=FALSE
    ),error=function(e){message("  df error [",sym,"]: ",e$message);NULL})
  })
  today_results<-bind_rows(Filter(Negate(is.null),today_results))
  lines <- readLines("~/Downloads/EdgeScreener2 2/app/01_fundamentals.R")
  idx <- grep("bind_rows\\(cached\\[", lines)
  lines[idx] <- "  final<-if(!is.null(cached)&&nrow(cached)>0)bind_rows(mutate(cached[!cached$symbol%in%today_tickers,],last_updated=as.character(last_updated)),today_results) else today_results"
  writeLines(lines, "~/Downloads/EdgeScreener2 2/app/01_fundamentals.R")
  message("Fixed!")
  source("01_fundamentals.R")
  fund_data <- run_module1(tickers=c("AAPL","MSFT","CRWD"), force_refresh=TRUE)final<-if(!is.null(cached)&&nrow(cached)>0)bind_rows(cached[!cached$symbol%in%today_tickers,],today_results) else today_results
  final<-final%>%arrange(symbol)
  write_csv(final,"data/fundamentals_scored.csv");write_csv(final,"app/fundamentals_scored.csv")
  saveRDS(list(last_run=Sys.Date(),batch=batch_idx+1),"data/batch_tracker.rds")
  saveRDS(yahoo_cache,"data/price_series_cache.rds")
  message("\n=== DONE === Stocks: ",nrow(final)," Regime: ",regime)
  final
}
